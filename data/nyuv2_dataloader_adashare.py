...
class NYU_v2(torch.utils.data.Dataset):
    ...
    def __scale__val__(img, label1, label2, label3, crop_h, crop_w):
        """
           Scales the images to crop size.
        """
        # random value between 0.5 and 1.5
    ...
    def __getitem__(self, item):
        ...
        labels = torch.from_numpy(np.setdiff1d(np.unique(seg), [255])).to(torch.int64)
        seg_labels = F.one_hot(labels, num_classes=40).sum(dim=0).float()
        
        batch = {'input': self.transform(img).float(), 
                 'segment_semantic': torch.from_numpy(seg).permute(2, 0, 1),
                 'seg_aux': seg_labels,
