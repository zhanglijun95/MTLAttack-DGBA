...
class DataMetrics(object):
    ...
    def reset_records(self):
        ...
        elif self.task == 'seg_aux':
            self.records = {'precision': [], 'recall': [], 'F1': []}
    ...
    def __seg_aux_error(self, pred, gt, threshold=0.5):
        pred = torch.sigmoid(pred).detach().cpu().numpy()
        pred = np.array(pred > threshold, dtype=float)
        gt = gt.detach().cpu().numpy()
        prec = precision_score(gt, pred, average='samples')
        recall = recall_score(gt, pred, average='samples'),
        f1 = f1_score(gt, pred, average='samples')
        return prec, recall[0], f1
    
    def __seg_aux_records(self, pred, gt, threshold=0.5):
        prec, recall, f1 = self.__seg_aux_error(pred, gt, threshold)
        self.records['precision'].append(prec)
        self.records['recall'].append(recall)
        self.records['F1'].append(f1)
    ...
    def __call__(self, pred, gt, mask=None):
        ...
        if self.task == 'segment_semantic':
            ...
            self.__seg_aux_records(pred, gt)
        return
...
class NYUMetrics(DataMetrics):
    ...
    def val_metrics(self):
        if self.task == 'segment_semantic':
            ...
            val_metrics = self.__seg_aux_metrics()
        self.reset_records()
        return self.round_dict(val_metrics)
...
#                        ((val_metrics['Pixel Acc'] - self.refer['Pixel Acc'] ) / self.refer['Pixel Acc'])) /2 
    ...
    def __seg_aux_metrics(self):
        val_metrics = {}
        val_metrics['precision'] = (np.array(self.records['precision']) * np.array(self.batch_size)).sum() / sum(self.batch_size)
        val_metrics['recall'] = (np.array(self.records['recall']) * np.array(self.batch_size)).sum() / sum(self.batch_size)
        val_metrics['F1'] = (np.array(self.records['F1']) * np.array(self.batch_size)).sum() / sum(self.batch_size)
        return val_metrics
